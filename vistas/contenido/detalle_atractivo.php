<main role="main">
  <section class="jumbotron text-center bg-light text-white bckgrnd-img" style="background-image: linear-gradient(#00000077, #000000bb), url('<?php echo $atractivo['thumbnail']; ?>');">
    <br><br><br><br>
    <div class="container">
      <h1 class="display-3"><?php echo $atractivo['nombre_atractivo']; ?></h1>
  ...
  <section class="container">
    <div class="row">
      <div class="col-12" style="margin: 0 auto; background-color: white; border-radius: 1rem; padding: 1.6rem 2.2rem; filter: drop-shadow(0 0.3rem 0.5rem #00000033);">
        <h5 class="lead"><b class="badge badge-primary">Lugar</b> <?php echo $atractivo['direccion']; ?></h5>
        <br>
        <br>
        <p class=""><?php echo $atractivo['descripcion']; ?></p>
      </div>
    </div>
  </section>
  ...
  <section class="container text-center">
    ...
    <form method="post" action="index.php?c=atractivo_controller&m=anadir_opinion&id_usuario=<?php if(isset($_SESSION['id_usuario']) && $_SESSION['id_usuario'] != '') { echo $_SESSION['id_usuario']; } else { echo '-1'; } ?>&id_atractivo=<?php echo $atractivo['id_atractivo'] ?>&recomendacion=5">
